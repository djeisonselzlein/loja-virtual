...
create table LSPS_MODULES (
    ...
    CREATE_PROCESS_LOG boolean not null,
...
create table LSPS_PERSONS (
    ...
    PHONE varchar(255),
    SUBSTITUTION_ACTIVE boolean not null,
    primary key (ID)
) ENGINE=InnoDB;
...
create table LSPS_TODO_PERFORMERS (
    ...
    TODO_FINISHED boolean not null,
    TODO_ISSUED_DATE datetime not null,
    TYPE varchar(20) not null,
    TODO_ID bigint not null,
    primary key (ID)
) ENGINE=InnoDB;
...
create table LSPS_WS_REQUEST_HANDLERS (
    ...
    REQUEST longtext,
    REQUEST_HEADERS longblob,
    REQUEST_RECEIVED datetime,
    RESPONSE longtext,
    RESPONSE_HEADERS longblob,
...
alter table LSPS_MONITORING_PERFORMERS 
    ...
    references LSPS_DASHBOARD_TABS (ID);

alter table LSPS_PERSON_DETAILS 
    add constraint FK_lbja4c4oq1uxade8ex1m5pgmj 
    foreign key (PERSON_ID) 
    references LSPS_PERSONS (ID);
...
alter table LSPS_PROCESS_ELEMENTS 
    ...
    references LSPS_PROCESS_ELEMENTS (ID);

alter table LSPS_PROCESS_ELEMENTS 
    add constraint FK_rqd9lixsx0143i5ixi9fhc7sr 
    foreign key (PROCESS_ID) 
    references LSPS_PROCESSES (ID);
...
create table LSPS_SEQ (
     next_val bigint 
);

insert into LSPS_SEQ values ( 2000 );

alter table LSPS_WS_REQUEST_HANDLERS add constraint FK_s65jik6djlb5eic0qv6b4yrpk foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_REST_RESOURCES add constraint FK_jrmcejvb7stvrwpqu6d3tgglr foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_REPORTS add constraint FK_7nm5khaodj4hmtesv9l94awvu foreign key (MODULE_ID) references LSPS_MODULES (ID);

alter table LSPS_TIMER_NOTIFICATIONS add constraint FK_ng6i84skd6gntvqq1uuaylf94 foreign key (INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_DOCUMENTS add constraint FK_20c0w8s92v5a3bkv5rcloi4qq foreign key (MODULE_ID) references LSPS_MODULES (ID);

alter table LSPS_TODOS add constraint FK_l7bt6coufivbuaovi3nppqcu7 foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_MODEL_UPDATES add constraint FK_spbwmt8kfuvkg0c86lllah8c5 foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_LOGS add constraint FK_imdgkti1qe5sfdudi6nqx7rny foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);

alter table LSPS_ROLLBACK_ITEMS add constraint FK_hjsguwmg9l9jx77mkeh7vw95k foreign key (MODEL_INSTANCE_ID) references LSPS_MODEL_INSTANCES (ID);
...
create function LSPS_ADD_SECONDS(d datetime, s int)
returns datetime deterministic
	return d + interval s second;

create function LSPS_DIFF_SECONDS(d1 timestamp, d2 timestamp)
returns bigint deterministic
	return unix_timestamp(d1) - unix_timestamp(d2);
