...
create table LSPS_MODULES (
    ...
    CREATE_PROCESS_LOG bit not null,
...
create table LSPS_PERSONS (
    ...
    PHONE varchar(255),
    SUBSTITUTION_ACTIVE bit not null,
    primary key (ID)
);
...
create table LSPS_TODOS (
    ...
    CURRENT_DELEGATION_LEVEL int not null,
    ELEMENT_ID bigint,
...
create table LSPS_WS_REQUEST_HANDLERS (
    ...
    REQUEST varchar(MAX),
    REQUEST_HEADERS varbinary(MAX),
    REQUEST_RECEIVED datetime,
    RESPONSE varchar(MAX),
    RESPONSE_HEADERS varbinary(MAX),
...
go
create function LSPS_DIFF_SECONDS(@d1 datetime, @d2 datetime) returns int as begin
	return datediff(second, @d1, @d2)
end;
go

create function LSPS_ADD_SECONDS(@d datetime, @s int) returns datetime as begin
	return dateadd(second, @s, @d)
end;
go
