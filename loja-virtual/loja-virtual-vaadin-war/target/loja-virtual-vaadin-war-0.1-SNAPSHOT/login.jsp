<%@ page contentType="text/html;charset=UTF-8" session="false" import="com.selzlein.lojavirtual.vaadin.util.Constants"%><%
...
<head>
	...
	function loadcssfile(filename){
		...
		if(typeof(Storage)!=='undefined'){
			var item = storage.getItem("loja-virtuallication-theme-name");
			if(item != undefined){
...
<tr class="v-formlayout-row v-formlayout-firstrow">
	<td class="v-formlayout-captioncell"><div class="v-caption">
		<label for="username" onclick=""><fmt:message key="login_username"/></label>
		<span class="v-required-field-indicator">*</span>
	</div></td>
	<td class="v-formlayout-errorcell"><div class="v-formlayout-error-indicator">
		<% if ((request.getParameterMap().containsKey("failed"))||((request.getQueryString() != null)&&(request.getQueryString().contains("failed")))) { %><div class="v-errorindicator" title="<fmt:message key="login_failed"/>">&nbsp;</div><% } %>
	</div></td>
	<td class="v-formlayout-contentcell" width="100%">
		<input type="text" placeholder="<fmt:message key="login_username"/>" class="v-textfield v-widget" name="j_username" id="username" autofocus="autofocus" autocapitalize="off" onkeypress="var code=event.keyCode||event.which;if(code===13){document.forms['loginform'].classList.remove('open'); setTimeout(function(){document.forms['loginform'].submit()},200);}return true" />
	</td>
</tr>
<tr class="v-formlayout-row">
	<td class="v-formlayout-captioncell"><div class="v-caption">
		<label for="password" onclick=""><fmt:message key="login_password"/></label>
		<span class="v-required-field-indicator">*</span>
	</div></td>
	<td class="v-formlayout-errorcell"><div class="v-formlayout-error-indicator"></div></td>
	<td class="v-formlayout-contentcell <% if ((request.getParameterMap().containsKey("failed"))||((request.getQueryString() != null)&&(request.getQueryString().contains("failed")))) { %>error" title="<fmt:message key="login_failed"/><% } %>" width="100%">
		<input type="password" placeholder="<fmt:message key="login_password"/>" class="v-textfield v-widget" name="j_password" id="password" onkeypress="var code=event.keyCode||event.which;if(code===13){document.forms['loginform'].classList.remove('open'); setTimeout(function(){document.forms['loginform'].submit();},200);}return true" />
	</td>
</tr>
...
<!-- CONTENT END -->
	<div id="container-foot">
		<div id="foot">
			<fmt:message key="lspsRelease"/> 3.0 &#160; (<fmt:message key="lspsBuild"/> 0.1-SNAPSHOT) <!-- Revision N/A --><br />
			&#0169; 2007-2014 Whitestein Technologies AG. <fmt:message key="allRightsReserved"/>
		</div>
	</div>
