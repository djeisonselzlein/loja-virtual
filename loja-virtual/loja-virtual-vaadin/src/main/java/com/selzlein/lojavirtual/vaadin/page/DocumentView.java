...
import com.selzlein.lojavirtual.vaadin.core.AppFormConnector;
...
import com.whitestein.lsps.common.LspsRuntimeException;
import com.whitestein.lsps.engine.lang.ExecutionContext;
import com.whitestein.lsps.human.UIImplementationType;
...
import com.whitestein.lsps.vaadin.LspsCustomUIWithContextDocumentHolder;
		...
		if (documentHolder != null) {
			...
			try {
				...
				if (documentId.matches("[0-9]+")) {
					...
					String implementationType = documentService.getImplementationType(savedDocumentId);
					UIImplementationType uiImplementationType = UIImplementationType.valueOf(implementationType);

					switch (uiImplementationType) {
					case VAADIN_MODEL_UI:
						documentHolder = new LspsDocumentHolder(savedDocumentId, ui.getAppConnector(), formConnector, userId);
						break;
					case VAADIN_CUSTOM_UI:
						documentHolder = new LspsCustomUIDocumentHolder(savedDocumentId, ui.getAppConnector(), formConnector, userId);
						break;
					case VAADIN_CUSTOM_UI_WITH_CONTEXT:
						documentHolder = new LspsCustomUIDocumentHolder(savedDocumentId, ui.getAppConnector(), formConnector, userId);
						break;
					default:
						throw new LspsRuntimeException("Unsupported implementation type " + uiImplementationType);
					}

				} else {
					String implementationType = documentService.getImplementationType(documentId);
					UIImplementationType uiImplementationType = UIImplementationType.valueOf(implementationType);
					...
					case VAADIN_CUSTOM_UI:
						...
						break;
					case VAADIN_CUSTOM_UI_WITH_CONTEXT:
						documentHolder = new LspsCustomUIWithContextDocumentHolder(documentId, params, ui.getAppConnector(), formConnector, userId);
						break;
					default:
						throw new LspsRuntimeException("Unsupported implementation type " + uiImplementationType);
					}
