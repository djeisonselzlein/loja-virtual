...
import com.selzlein.lojavirtual.vaadin.popup.TodoDetails;
...
import com.whitestein.lsps.vaadin.LspsCustomUIWithContextTodoHolder;
	...
	public void enter(ViewChangeEvent event) {
		if (todoHolder != null) {
			...
			try {
				...
				String implementationType = todoService.getImplementationType(todoId);
				UIImplementationType todoImplementationType = UIImplementationType.valueOf(implementationType);

				switch (todoImplementationType) {
				...
				case VAADIN_CUSTOM_UI_WITH_CONTEXT:
					todoHolder = new LspsCustomUIWithContextTodoHolder(todoId, ui.getAppConnector(), formConnector, userId, readOnly);
					break;
				case VAADIN_CUSTOM_UI:
					todoHolder = new LspsCustomUITodoHolder(todoId, ui.getAppConnector(), formConnector, userId, readOnly);
					break;
				default:
					throw new LspsRuntimeException("Unsupported todo implementation type: " + todoImplementationType);
				}
